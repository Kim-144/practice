(function () {
  ...
  */
  ...
  _.each = function (collection, iterator) {
    // collection 이 배열일 경우, 값이랑 인덱스가 iterator를 통해서 콜렉션에 짝지어 담긴다. 
    // collection 이 객체일 경우, 키랑 밸류 값이 iterator를 통해서 콜렉션에 짝지어 담긴다. 
  ...
  _.reject = function (collection, test) {
     ...
     */
    //filter함수를 이용해서 요소들을 조회할때 
    //true -> collection[i] 리턴하지 않는다.
  ...
  _.uniq = function (array) {
    //array 의 요소들을 중복하지 않는 새로운 배열을 만든다.
    // 배열의 요소들을 조회하고 기존의 요소가 새로운 배열에 포함되지 않을 경우 새로운 배열에 넣어준다
    let arrayUniq = [];

    _.each(array, function (i) {
      if (!arrayUniq.includes(i)) {
        arrayUniq.push(i);
      }
    });
    return arrayUniq;
  ...
  _.some = function (collection, iterator) {
    ...
    for (let i = 0; i < collection.length; i++)
      if (iterator) {
        if (iterator(collection[i])) result = true;
      }
      else if (collection[i]) result = true;
    return result;
  };
  ...
  */
  ...
  _.once = function (func) {
    ...
    return function () {
      ...
      if (alreadyCalled) { return result }
      alreadyCalled = true;
  ...
  */
  ...
  _.invoke = function (collection, functionOrKey, args) {
    ...
    if (typeof (functionOrKey) === 'string') {
      ...
      _.each(collection, function (i) {
        arr.push(functionOrKey.apply(i));
      });
    }
    return arr;
  }
